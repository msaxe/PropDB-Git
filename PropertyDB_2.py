...
class Application(tk.Frame):
	...
	def createMenu(self):
		...
		docmenu.add_command(label="Help Document", command=lambda : self.open_help_document(paths.resource_path('AboutDocs/PropertyDB_HowTo.pptx')))
		docmenu.add_command(label="Back End Doc" , command=lambda : self.open_help_document(paths.resource_path('AboutDocs/PropertyDB_BackEnd.docx')))
		docmenu.add_command(label="Known Issues" , command=lambda : self.open_help_document(paths.resource_path('AboutDocs/PropertyDB_Fixes-Future.docx')))
	...
	def createWidgets(self):
		...
		self.addplusimage = PIL.ImageTk.PhotoImage(PIL.Image.open(paths.resource_path(r"icons\addplus_30_2.png")))
		...
		self.editcirimage = PIL.ImageTk.PhotoImage(PIL.Image.open(paths.resource_path(r"icons\editpenc_30.png")))
		...
		self.deldashimage = PIL.ImageTk.PhotoImage(PIL.Image.open(paths.resource_path(r"icons\delex_30.png")))
		...
		self.exportimage = PIL.ImageTk.PhotoImage(PIL.Image.open(paths.resource_path(r"icons\export_30_2.png")))
	...
	def createSideBar(self):
		...
		def mouse_motion(event):
			info_h,in_req = self.midmainframe.winfo_width() , self.midmainframe.winfo_reqwidth()
			imga_h,im_req = self.midside.winfo_width() ,self.midside.winfo_reqwidth()
			min__h = self.mid_move_button.winfo_width()
			total  = self.midframe.winfo_width()
			
			print event, info_h, in_req, imga_h, im_req, total, min__h
			
			if info_h + event > min__h and imga_h - event > min__h:
				self.midside		.configure(width=imga_h - event)
				self.midmainframe	.configure(width=info_h + event)
			elif info_h + event < min__h:
				self.midmainframe	.configure(width=min__h)
				self.midside		.configure(width=total - min__h)
			elif imga_h - event < min__h:
				self.midmainframe	.configure(width=total - min__h)
				self.midside		.configure(width=min__h)
			
			self.midframe.update_idletasks()
			print event, self.midmainframe.winfo_width(), self.midside.winfo_width(), self.midframe.winfo_width(), self.mid_move_button.winfo_width()
	
		# mlb data section
		self.midmainframe = tk.Frame(self.midframe)
		self.midmainframe.configure(bg=main_bg_color)
				
		#vertical move bar section
		self.movebar_vert_image = PIL.ImageTk.PhotoImage(PIL.Image.open(paths.resource_path(r"icons\movebar_vert.png")))
		self.mid_move_button = tk.Label(self.midframe, image=self.movebar_vert_image,width=6,height=80)
		self.mid_move_button.configure(bg="#EAEAEA",relief=tk.FLAT,cursor='sb_h_double_arrow')
		self.mid_move_button.bind('<ButtonRelease-1>' ,lambda e: mouse_motion(e.x))
		
		# side info bar section
		self.midside = tk.Frame(self.midframe)
		self.midside.configure(bg=main_bg_color)
		
		#generate side bar
		self.side_info = SB.SideBar_Info(self.midside,self)
		
		# place widgets in grid
		#self.midmainframe	.grid(column=0,row=0,sticky='nsew')
		#self.mid_move_button.grid(column=1,row=0,sticky='ns')
		#self.midside		.grid(column=2,row=0,sticky='nsew')
		self.midmainframe	.pack(side=tk.LEFT, expand=tk.YES, fill=tk.BOTH)
		self.mid_move_button.pack(side=tk.LEFT, expand=tk.NO, fill=tk.Y)
		self.midside		.pack(side=tk.LEFT, expand=tk.YES, fill=tk.BOTH)
		
		# configure rows and columns
		#self.midframe.grid_columnconfigure(2,weight=2)
		#self.midframe.grid_columnconfigure(0,weight=3)
	...
	def load_table(self,flds,table,constraints='',but=0,check_date=True):
		...
		self.mlb = MLB.MultiListbox(self.midmainframe, column_names , self)
		...
		self.side_info.load_label_rows(self.db.fetchAliases(table),table)
		return
	...
	def update_sel_count(self,event=0):
		...
		if self.side_info:
			try:
				pkeys = [self.mlb.indexes[s] for s in self.mlb.curselection()]
				self.side_info.load_selected_keys( pkeys )
			except:
				pass
...
if __name__ == '__main__':
	...
	try:
		app.master.iconbitmap(default=paths.resource_path(r"icons\icon.ico"))
	except:
